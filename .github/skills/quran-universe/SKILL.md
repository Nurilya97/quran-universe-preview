...
- the neon lemon remains the approved **Structure** navigation accent for the Word Orbit “Структура слова” node and the active underline/indicator inside Word Structure;
- all Russian/UI labels such as “Разбор”, “Схема”, “Форма”, “Именная форма” and “Именная модель” stay white, including the text of the Form legend item;
- the Form legend dot/marker may stay `#EAFF5B`, but its Russian label must not inherit that color;
- only the displayed Arabic form/pattern itself — for example `فَعْلَى` — uses `#EAFF5B` inside Word Structure; explanatory prose remains neutral so the accent keeps semantic meaning rather than becoming decoration;
